                  ...
                  removeUserFromWaitList(user);
						...
						if (onlineUsersForCurrGame.size() % GAME_MEMBER_CNT != 0) { //check waiting list
							...
							if (filteredWaitList.size() < need) {
								...
								int extraCnt = onlineUsersForCurrGame.size() % GAME_MEMBER_CNT;
								List<String> list = signUpInfo.get(time + halfHour);
								if (list == null) 
									list = new ArrayList<String>();
								for (int i = 0; i < extraCnt; i++) {
									int size = onlineUsersForCurrGame.size();
									String userName = onlineUsersForCurrGame.get(size - i - 1);
									if (!list.contains(userName))
									  list.add(userName);
								}							
								signUpInfo.put(time + halfHour, list);
							} else {
